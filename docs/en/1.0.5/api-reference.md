...
| `queue_type`              | `QueueType \| None` | `None`   | Sets `x-queue-type` on declaration. `None` uses the broker's `default_queue_type`. |
...
Enum of the RabbitMQ queue types django_rmq knows how to declare; its value is serialized to the broker as the `x-queue-type` argument.
...
`QueueType` subclasses `str` (instead of the 3.11+ `StrEnum`, unavailable on Python 3.10), so each member is an actual string and serializes to the broker as-is. When `queue_type` is left unset on a `QueueConfig`, the broker applies its own `default_queue_type`.
...
Builds the AMQP `arguments` dict for queue declaration from `queue_type`
(`x-queue-type`) and the dead-letter fields. Returns `None` when none of them
are set.
...
`AppConfig.ready()`. See [Configuration](/en/1.0.5/configuration.html) for a full
settings example.
