...
- **`QueueConfig`** — a frozen dataclass that carries queue declaration parameters (name, durability, dead-letter settings) used by both producers and consumers.
...
The `.arguments` property builds the `x-dead-letter-*` dict for `queue_declare`. It returns `None` when no dead-letter fields are set, which avoids passing an empty `arguments` dict to the broker.
...
See [Management Commands](/en/1.0.4/management-commands.html) for the full command reference.
...
- [Producers](/en/1.0.4/producers.html) — how `QueueConfig` affects queue declaration in the producer.
- [Consumers](/en/1.0.4/consumers.html) — how `QueueConfig` affects queue declaration in the consumer.
- [Registries](/en/1.0.4/registries.html) — `SetupRegistry` lifecycle and `get_setup_registry`.
- [Management Commands](/en/1.0.4/management-commands.html) — `setup_rabbitmq_topology` reference.
- [Reliability](/en/1.0.4/reliability.html) — dead-letter delivery guarantees.
- [API Reference](/en/1.0.4/api-reference.html) — `QueueConfig` and `SetupRegistry` signatures.
