...
- [Топология](/ru/topology.html) — объявление exchanges и очередей с помощью `QueueConfig` и setup-функций.
- [Надёжность](/ru/reliability.html) — полная модель доставки, подтверждения и детали переподключения.
- [Несколько подключений](/ru/multiple-connections.html) — параметр `using`.
- [Справочник API](/ru/api-reference.html) — полная сигнатура `Producer`.
