...
- [Топология](/ru/1.0.5/topology.html) — объявление exchanges и очередей с помощью `QueueConfig` и setup-функций.
- [Надёжность](/ru/1.0.5/reliability.html) — полная модель доставки, подтверждения и детали переподключения.
- [Несколько подключений](/ru/1.0.5/multiple-connections.html) — параметр `using`.
- [Справочник API](/ru/1.0.5/api-reference.html) — полная сигнатура `Producer`.
