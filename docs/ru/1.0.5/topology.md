...
- **`QueueConfig`** — `frozen dataclass`, содержащий параметры объявления очереди (имя, долговечность, тип очереди,
  настройки dead-letter), используемый как продюсерами, так и потребителями.
...
| `queue_type`              | `QueueType \| None` | `None`       | Устанавливает `x-queue-type` при объявлении; при `None` используется `default_queue_type` брокера. |
...
Свойство `.arguments` формирует словарь `x-queue-type` и `x-dead-letter-*` для `queue_declare`. Возвращает `None`, если
ни одно из этих полей не задано, что позволяет избежать передачи пустого словаря `arguments` брокеру.
...
| **Тип очереди**    | Поддерживается (через `queue_type` / `x-queue-type`)                          | Не поддерживается (дефолт брокера)                                                       |
...
См. [Management-команды](/ru/1.0.5/management-commands.html) для полного справочника команд.
...
- [Продюсеры](/ru/1.0.5/producers.html) — как `QueueConfig` влияет на объявление очереди в продюсере.
- [Потребители](/ru/1.0.5/consumers.html) — как `QueueConfig` влияет на объявление очереди в потребителе.
- [Реестры](/ru/1.0.5/registries.html) — жизненный цикл `SetupRegistry` и `get_setup_registry`.
- [Management-команды](/ru/1.0.5/management-commands.html) — справочник по `setup_rabbitmq_topology`.
- [Надёжность](/ru/1.0.5/reliability.html) — гарантии доставки dead-letter.
- [Справочник API](/ru/1.0.5/api-reference.html) — сигнатуры `QueueConfig` и `SetupRegistry`.
